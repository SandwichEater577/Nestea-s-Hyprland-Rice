...
def network_status():
    rows = [fields(l) for l in nm('-t', '-f', 'DEVICE,TYPE,STATE,CONNECTION', 'device').splitlines()]
    live = [r for r in rows if len(r) == 4 and r[2] == 'connected' and r[1] in ('ethernet', 'wifi')]
    live.sort(key=lambda r: r[1] != 'ethernet')
    if not live:
        print(json.dumps({'text': '󰖪', 'tooltip': 'No internet\nRight-click: networks'})); return
    dev, kind, _, name = live[0]
    counter = int((Path('/sys/class/net')/dev/'statistics/rx_bytes').read_text())
    cache = STATE/'network-rate.json'
    now = time.monotonic()
    rate = 0
    try:
        prev = json.loads(cache.read_text())
        if prev['dev'] == dev and now > prev['time']:
            rate = max(0, (counter-prev['bytes'])/(now-prev['time'])/1_000_000)
    except (OSError, ValueError, KeyError): pass
    cache.write_text(json.dumps({'dev': dev, 'bytes': counter, 'time': now}))
    connectivity = nm('networking', 'connectivity')
    signal = ''
    if kind == 'wifi':
        for line in nm('-t', '-f', 'IN-USE,SIGNAL', 'device', 'wifi', 'list', '--rescan', 'no', 'ifname', dev).splitlines():
            if line.startswith('*:'): signal = line.split(':')[1] + '% '
    icon = '󰖪' if connectivity in ('none', 'limited', 'portal') else ('󰈀' if kind == 'ethernet' else '')
    address = nm('-g', 'IP4.ADDRESS', 'device', 'show', dev)
    print(json.dumps({'text': f'{icon}\u2002\u2002{signal} ↓  {rate:.2f} MB/s', 'tooltip': html.escape(f'{name} · {dev}\n{address}\nInternet: {connectivity}\nRight-click: networks and diagnostics')}))
